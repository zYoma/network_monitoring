...
print "Content-type: text/html\n\n";
 $my_cgi = new CGI;
# Conf
my $pingtimeout = '0.200'; 				# зависит от ресурсов машины и пинга до оборудовани€, примерно должно быть пинг_до_оборудовани€+150мс
my $snmpport = '161';
...
"10.228.73.82-10.228.73.82",
"10.228.73.83-10.228.73.83",
...
#				print "\n $sysname lldpneoif= $lldpneioid\n";
				...
				if($id >=0 && $id <= 99999)
					{
					my $myport = $&;
					if ($id == 12 || $id == 17 || $id == 19 ){$myport= $myport+4;}#дл€ хуавеев
					if ($id == 112){$myport= $myport+5;}#дл€ хуавеев
...
foreach $iphost(keys %database)
	...
	# ≈сли какой придурок напихал кавычек в переменную
	$syslocation =~ s/"/\\"/g; 
	$sysname =~ s/"/\\"/g; 
	
	# ≈сли какой нибудь мудак напихал \n в названи€
	$syslocation =~ s/\\n/\\\\n/g; 
	$sysname =~ s/\\n/\\\\n/g; 	
