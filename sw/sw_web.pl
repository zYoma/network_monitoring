...
elsif ($result->{$snmp_oid} eq ".1.3.6.1.4.1.2011.2.23.92" || $result->{$snmp_oid} eq ".1.3.6.1.4.1.2011.2.23.80" || $result->{$snmp_oid} eq ".1.3.6.1.4.1.2011.2.23.229" || $result->{$snmp_oid} eq ".1.3.6.1.4.1.2011.2.23.427")
	{
	$width ='425';
...
elsif ( $result->{$snmp_oid} eq ".1.3.6.1.4.1.3807.1.482816" || $result->{$snmp_oid} eq ".1.3.6.1.4.1.3807.1.485201")
	{
	$width ='425';
	# $temOid= '.1.3.6.1.4.1.2011.5.25.31.1.1.1.1.11.67108873';
	# $getFireware = '1.3.6.1.2.1.47.1.1.1.1.10.67108867';
	# $getSn = '1.3.6.1.2.1.47.1.1.1.1.11.67108873';
	$getHashUn ='.1.3.6.1.4.1.3807.1.8012.3.2.2.1.3';
	$getHash ='.1.3.6.1.4.1.3807.1.8012.3.2.2.1.2';
	$id = 36;
	}
...
if ($id == 4 || $id == 5 || $id ==6 || $id ==8 || $id == 14 || $id == 15){
	...
	if ($id == 1 || $id == 2 || $id ==3 || $id ==7 || $id == 9 || $id == 11|| $id == 24|| $id == 25|| $id == 26  || $id == 27  || $id == 28  || $id == 29  || $id == 30 || $id == 31|| $id == 32 || $id == 33 || $id == 34 || $id == 35 ){
...
if ($id == 24 || $id == 25 || $id == 31 || $id == 32 || $id == 35 || $id == 36 ){
	$device =~ /^(.+)\s/;
	$device = $1;
	}
	if ($id == 26 ){
	...
	if ($id == 36 ){$device = "FiberHome S4820-28T-X";}
	if ($id == 22 || $id == 23 ){
	$device = "H3C";
	}
#get sysName
...
$snmptemp = $session -> get_request(".1.3.6.1.4.1.171.10.75.15.3.100.1.1.0");
 ...
 eval{
  ...
  elsif ($id == 24 || $id == 25 || $id == 31 || $id == 32 || $id == 35){
eval{
...
$snmptemp = $session -> get_request("1.3.6.1.4.1.171.12.1.1.6.1.0");
 ...
 #get IP
 eval{
 if ($id == 24 || $id == 25 || $id == 27 || $id == 32 || $id == 35 || $id == 36){
...
#Получаем Шлюз
	...
	foreach my $oid (oid_lex_sort(keys(%{$session->var_bind_list}))) {
		...
		if ($name_port =~/Loop/ || $name_port =~/Vlan/ || $name_port =~/NULL/ ) {next;}
			if ($name_port =~/GigabitEthernet0\/0\/1/) {$id_port  = 25; $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/2/) {$id_port  = 26;  $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/3/) {$id_port  = 27;  $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/4/) {$id_port  = 28;  $nameMac{ $id_port } = $name_port;}
			else{
				my $kostil = $name_port;
				$kostil =~ s/\/(\d+)$/$1/;
				  $id_port = $1;
				 $nameMac{ $id_port } = $name_port;
				}
		}
		...
		my $id_port;
		if ($name_port =~/Loop/ || $name_port =~/Vlan/ || $name_port =~/NULL/ ) {next;}
			if ($name_port =~/GigabitEthernet0\/0\/1/) {$id_port  = 49; $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/2/) {$id_port  = 50;  $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/3/) {$id_port  = 51;  $nameMac{ $id_port } = $name_port;}
			elsif ($name_port =~/GigabitEthernet0\/0\/4/) {$id_port  = 52;  $nameMac{ $id_port } = $name_port;}
			else{
				my $kostil = $name_port;
				$kostil =~ s/\/(\d+)$/$1/;
				  $id_port = $1;
		...
		if ( $id == 32) {
		#Костыль для хуавея Есенина 131
		
			if ($temp_port == 116 || $temp_port == 117 || $temp_port == 118 || $temp_port == 119) {$temp_port=$temp_port-106;}
			
			else {
				if  ($temp_port < 58 ) {$temp_port = $temp_port + 191;}
				elsif ($temp_port > 57 && $temp_port < 108) {$temp_port = $temp_port + 243;}
			}
	 }
...
eval{	
 ...
 %snmplldpnei_hash = %{$session -> get_table("$rootPortOid")};
	...
	foreach my $oid (oid_lex_sort(keys(%{$session->var_bind_list}))) {
		...
		my $lldp_port = $1;
		my $interface = $hash{$lldp_port};
		
		$lldp = $session->var_bind_list->{$oid};
			
			if ($id == 24 || $id == 35 ){
			$interface = $nameMac{$lldp_port};
			}
		
		if ($lldp_port eq $rootPort){
		
		push @lldpDB,"<form method='post' action='sw_web.pl'>$interface:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='green'>$lldp</font> - <font  color='#0489B1'>$hashLLDPdescription{$lldp_port}</font> - <font color='red'><b>root    </b></font><button type=\"submit\" class=\"btn btn-outline-danger btn-sm\" style=\"padding: 0 3px ;\"  value=\"$lldp\" name=\"name\">></button> </form>";

		
		}
		else {
		push @lldpDB,"<form method='post' action='sw_web.pl'>$interface:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='green'>$lldp</font> - <font  color='#0489B1'>$hashLLDPdescription{$lldp_port}   </font><button type=\"submit\" class=\"btn btn-outline-danger btn-sm\" style=\"padding: 0 3px ;\"  value=\"$lldp\" name=\"name\">></button></form>";
		}
...
my $last_number = 0;
	for (my $i = 0; $i < scalar(@hash); $i++ ){
		if ($hash[$i] == 1) {
		my $pnumber = $i+1;
		if ( $id == 24 || $id == 25 || $id == 32 || $id == 35){$pnumber = $i;}
...
FOR:
	for (my $i = 0; $i < scalar(@hash); $i++ ){
		...
		my $pnumber = $i+1;
		if ( $id == 24 ||  $id == 32 || $id == 35){$pnumber = $i;}
		if ( $id == 4 ||  $id == 5 ||  $id == 6 ||  $id == 8 ||   $id == 16){$pnumber = $pnumber - 32;}
...
<br>
  ...
  <form action="mac_web.pl" target="area" style="float: right;">
   ...
   <form action="mapnet.pl" target="blank" style="float: right; margin-right: 10px;">
   <button type="submit" class="btn btn-danger" value="$input" name="name">map</button>
  </form>
  ~;
