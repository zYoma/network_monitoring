...
print " <h2>Приемник недоступен</h2>";
$session->close();  
exit;
}
...
{
my $snmptemp = $session -> get_request('.1.3.6.1.4.1.11195.1.5.2.0');
 my $soft = $snmptemp -> {'.1.3.6.1.4.1.11195.1.5.2.0'};
 if ( $soft eq "1.4.5")
{
print "<h2><div class=\"layer\">Версия ПО актуальна.</div></h2>";
$session->close();  
exit;
}
else 
...
my( @list);
   push( @list, (".1.3.6.1.4.1.11195.1.1.10.0", OCTET_STRING,"LambdaPro71_ver_1.4.5.upg"));
     push( @list, (".1.3.6.1.4.1.11195.1.1.8.0", IPADDRESS, "10.40.254.109"));
     push( @list, (".1.3.6.1.4.1.11195.1.1.6.0", INTEGER, 1));
	 
 

  my $result = $session->set_request( -varbindlist => [@list]);

  print "<h2><div class=\"layer\">Обновлено.Перезагружается.</div></h2>";
  $session->close();  
}
}
#sdo3002
...
{
$snmptemp = $session -> get_request('.1.3.6.1.4.1.32108.1.7.4.2.3.0');
 my $comRW = $snmptemp -> {'.1.3.6.1.4.1.32108.1.7.4.2.3.0'};
...
my( @list);
    push( @list, (".1.3.6.1.4.1.32108.1.7.4.6.1.0", OCTET_STRING, "10.40.254.109"));
     push( @list, (".1.3.6.1.4.1.32108.1.7.4.6.2.0", INTEGER, 129));
  ...
  my $result = $session->set_request( -varbindlist => [@list]);
    ...
    sleep 20;
  my( @list2);
  push( @list2, (".1.3.6.1.4.1.32108.1.7.4.6.4.0", INTEGER, 129));
  $result = $session->set_request( -varbindlist => [@list2]);
   print "<h2><div class=\"layer\">Обновлено.Перезагружается.</div></h2>";
$session->close();  
}
}
...
{
$snmptemp = $session -> get_request('.1.3.6.1.4.1.32108.1.8.4.2.3.0');
 my $comRW = $snmptemp -> {'.1.3.6.1.4.1.32108.1.8.4.2.3.0'};
...
my( @list);
    push( @list, (".1.3.6.1.4.1.32108.1.8.4.6.1.0", OCTET_STRING, "10.40.254.109"));
     push( @list, (".1.3.6.1.4.1.32108.1.8.4.6.2.0", INTEGER, 129));
  ...
  my $result = $session->set_request( -varbindlist => [@list]);
    ...
    sleep 20;
  my( @list2);
  push( @list2, (".1.3.6.1.4.1.32108.1.8.4.6.4.0", INTEGER, 129));
  $result = $session->set_request( -varbindlist => [@list2]);
   print "<h2><div class=\"layer\">Обновлено.Перезагружается.</div></h2>";
$session->close();  
}
...
{
print "<h2><div class=\"layer\">Нового ПО нет.</div></h2>";
}
...
my( @list);
   push( @list, (".1.3.6.1.4.1.32108.1.9.4.6.1.0", OCTET_STRING, "10.40.254.109"));
     push( @list, (".1.3.6.1.4.1.32108.1.9.4.6.2.0", INTEGER, 129));
  ...
  sleep 20;
  my( @list2);
  push( @list2, (".1.3.6.1.4.1.32108.1.9.4.6.4.0", INTEGER, 129));
  $result = $session->set_request( -varbindlist => [@list2]);
 print "<h2><div class=\"layer\">Обновлено.Перезагружается.</a></div></h2>";
  $session->close();  
}
else 
{
